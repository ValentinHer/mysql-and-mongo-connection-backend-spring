...
public class S3StorageServiceImpl implements CloudStorageService {
	...
	// Guardar archivo en AWS S3 a traves del S3Client
	@Override
	public void uploadPerfilImage(MultipartFile file, String filename) {
		...
		try {
			...
			s3Client.putObject(putObject, RequestBody.fromBytes(imageContent));
			...
			logger.error("Error al procesar el archivo {}: {}", filename, e.getMessage(), e);
			throw new CloudStorageException("Error al procesar el archivo: " + e.getMessage(), e);
	...
	// Obtener una URL firmada para un archivo almacenado en un bucket de AWS S3
	@Override
	public String getPerfilImagePresignedUrl(String filename) {
	...
	// Eliminar archivo almacenado en AWS S3
	@Override
	public void deletePerfilImage(String filename) {
